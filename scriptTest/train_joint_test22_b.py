...
def main():
    ...
    TOTAL_EPOCHS = 40
    STAGE_1_EPOCHS = 10 # 前 10 個 Epoch 只訓練 RefineNet
    ...
    for epoch in range(TOTAL_EPOCHS):
        # 🌟 階段一：強制凍結 FusionNet，逼迫 RefineNet 學會真正對齊像素
        is_stage_1 = epoch < STAGE_1_EPOCHS
        
        if is_stage_1:
            print(f"\n--- [Stage 1] 第 {epoch+1} 回合：純幾何特訓 (凍結 FusionNet) ---")
            fusion_model.eval()
            refine_model.train()
            for p in fusion_model.parameters(): p.requires_grad = False
            for p in refine_model.parameters(): p.requires_grad = True
        else:
            # 🌟 階段二：全面解封，進行聯合訓練
            if epoch == STAGE_1_EPOCHS:
                print(f"\n🔥 [解封！] 第 {epoch+1} 回合：啟動 Stage 2 全局聯合訓練 🔥")
            else:
                print(f"\n--- [Stage 2] 第 {epoch+1} 回合：全局聯合優化 ---")
            fusion_model.train()
            refine_model.train()
            for p in fusion_model.parameters(): p.requires_grad = True
            for p in refine_model.parameters(): p.requires_grad = True
        ...
        for task in train_tasks:
            ...
            stage_prefix = "[S1-Refine]" if is_stage_1 else "[S2-Joint]"
            pbar = tqdm(range(frameCount), desc=f"{stage_prefix} Epoch {epoch+1}/{TOTAL_EPOCHS} | {task['name']}(QP{task['qp']})", unit="frame")
            ...
            for t in pbar:
                ...
                with torch.amp.autocast('cuda'):
                    ...
                    if is_stage_1:
                        # [Stage 1] 只有 Warp Loss 和 Smooth Loss，完全不理會 Pixel Loss
                        loss = loss_warp_direct + 0.05 * loss_smooth
                        loss_pixel_val = 0.0 # 僅供顯示
                    else:
                        # [Stage 2] 解封 FusionNet，計算最終 GT 差距
                ...
                if is_stage_1:
                    pbar.set_postfix({'Warp': f"{loss_warp_direct.item():.4f}", 'Smooth': f"{loss_smooth.item():.4f}"})
                else:
                    pbar.set_postfix({'Pix': f"{loss_pixel_val:.4f}", 'Warp': f"{loss_warp_direct.item():.4f}"})
                
            torch.cuda.empty_cache()

        scheduler.step()
        
        # S1 階段只存 RefineNet，S2 才兩者皆存，確保你中途暫停也不會存入未經訓練的假權重
