...
print(device)
model_path = "joint_fusion_net_test16.pth"

# ==========================================
# 1. 網路架構 (擴充至 8 通道，容納兩視圖幾何導引)
# ==========================================
...
def compute_epipolar_error_map(flow, shape):
    ...
    if F_mat is not None and F_mat.shape == (3, 3):
        ...
        # 物理優化：將硬性剪裁換成 Tanh 軟飽和，穩定 conv1 的輸入數值域
        err_map = np.tanh(err_map / 4.0)
        return err_map[..., np.newaxis]
    else:
        return np.zeros((H, W, 1), dtype=np.float32)

def ml_fuse_channel(t_up, t_p, t_n, flow_p, flow_n, flow_consistency, laplacian_guide, epi_p, epi_n):
    ...
    if epi_p.shape[2] != h or epi_p.shape[3] != w:
        epi_p = F.interpolate(epi_p, size=(h, w), mode='bilinear', align_corners=False)
    if epi_n.shape[2] != h or epi_n.shape[3] != w:
        epi_n = F.interpolate(epi_n, size=(h, w), mode='bilinear', align_corners=False)

    fused, _ = ml_model(t_up, warped_p, warped_n, flow_consistency, laplacian_guide, epi_p, epi_n)
    return fused
            ...
            for qp in config['qps']:
                ...
                with open(out_file, 'wb') as f_out:
                    for t in tqdm(range(frameCount), desc=f"Video: {video_name} QP: {qp}", unit="frame"):
                        ...
                        # 【新增】計算前、後向的兩視圖幾何對極誤差圖
                        epi_p_small = compute_epipolar_error_map(flow_p_small, up_8b.shape)
                        epi_n_small = compute_epipolar_error_map(flow_n_small, up_8b.shape)
                        ...
                        # 交叉融合與通道獨立保護
                        fused_y = ml_fuse_channel(t_up_y, t_p_y, t_n_y, flow_p_4k, flow_n_4k, flow_consistency_4k, t_lap_4k, epi_p_4k, epi_n_4k)
                        fused_u = ml_fuse_channel(t_up_u, t_p_u, t_n_u, flow_p_chroma, flow_n_chroma, flow_consistency_4k, t_lap_4k, epi_p_4k, epi_n_4k)
                        fused_v = ml_fuse_channel(t_up_v, t_p_v, t_n_v, flow_p_chroma, flow_n_chroma, flow_consistency_4k, t_lap_4k, epi_p_4k, epi_n_4k)
