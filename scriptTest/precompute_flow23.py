...
def get_small_flow_img_clahe(tensor_4k):
    """ 🌟 加入 CLAHE 處理，消除 Zombie 光影變換對 Farneback 的嚴重干擾 """
...
def main():
    ...
    for task in train_tasks:
        ...
        pbar = tqdm(range(frameCount), desc=f"抽取等速幾何庫 {task['name']}_QP{task['qp']}")
        for t in pbar:
            ...
            with torch.no_grad():
                ...
                # 僅使用高清的 Prev 與 Next 圖像來計算基準光流，捨棄 up_8b 的失真
                p_8b = get_small_flow_img_clahe(t_p_y)
                n_8b = get_small_flow_img_clahe(t_n_y)
            
            # 🌟 核心修改：計算間隔兩幀 (t-1 到 t+1) 的清晰雙向光流
            # 因物體位移變大，調高 levels(5->6), poly_n(5->7), poly_sigma(1.2->1.5) 來增加感受野
            flow_p2n = cv2.calcOpticalFlowFarneback(p_8b, n_8b, None, 0.5, 6, 25, 3, 7, 1.5, 0)
            flow_n2p = cv2.calcOpticalFlowFarneback(n_8b, p_8b, None, 0.5, 6, 25, 3, 7, 1.5, 0)
            
            # 🌟 假設短時間內為「等速線性運動」，將光流減半，當作 t_up 的乾淨基準光流 (Prior)
            # 這些基準光流會在後續交給 RefineNet 搭配真實的 t_up 來修正非線性的微小偏差
            flow_p = flow_n2p * 0.5   # Up 尋找 P，相當於 N 尋找 P 的一半
            flow_n = flow_p2n * 0.5   # Up 尋找 N，相當於 P 尋找 N 的一半
            flow_pb = flow_p2n * 0.5  # P 尋找 Up，相當於 P 尋找 N 的一半
            flow_nb = flow_n2p * 0.5  # N 尋找 Up，相當於 N 尋找 P 的一半
