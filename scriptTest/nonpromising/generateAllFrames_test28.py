...
class SEBlock(nn.Module):
    ...
    def forward(self, x): return x * self.fc(x).view(x.shape[0], x.shape[1], 1, 1)
...
luma_model = LumaHeavyFusionNet().to(device)
if os.path.exists(FUSION_MODEL_PATH) and os.path.exists(REFINE_MODEL_PATH):
    luma_model.load_state_dict(torch.load(FUSION_MODEL_PATH, map_location=device, weights_only=True))
...
def scale_flow(flow, h, w):
    scale_h, scale_w = float(h) / 540.0, float(w) / 960.0
    flow_large = F.interpolate(flow, size=(h, w), mode='bilinear', align_corners=False)
    flow_large[:, 0, :, :] *= scale_w
    flow_large[:, 1, :, :] *= scale_h
    return flow_large
            ...
            for qp in config['qps']:
                ...
                with open(out_file, 'wb') as f_out:
                    for t in tqdm(range(frameCount), desc=f"Video: {video_name} QP: {qp}", unit="frame"):
                        ...
                        t_up_y_small = F.interpolate(t_up_y, size=(540, 960), mode='bilinear', align_corners=False)
                        t_p_y_small = F.interpolate(t_p_y, size=(540, 960), mode='bilinear', align_corners=False)
                        t_n_y_small = F.interpolate(t_n_y, size=(540, 960), mode='bilinear', align_corners=False)
                        
                        flow_p_cv, flow_n_cv = flow_data_pt['fp'].float(), flow_data_pt['fn'].float()
                        flow_p_back_cv, flow_n_back_cv = flow_data_pt['fpb'].float(), flow_data_pt['fnb'].float()
                        epi_p_cv_t, epi_n_cv_t = flow_data_pt['ep'].float(), flow_data_pt['en'].float()
                        ...
                        flow_p_back_refined = refine_model(t_p_y_small, t_up_y_small, flow_p_back_cv)
                        flow_n_back_refined = refine_model(t_n_y_small, t_up_y_small, flow_n_back_cv)
                        ...
                        # 2. Luma (Y) 重武裝推論
                        flow_p_4k = scale_flow(flow_p_t_refined, 2160, 3840)
                        flow_n_4k = scale_flow(flow_n_t_refined, 2160, 3840)
                        warped_p_y = warp_tensor_by_grid(t_p_y, flow_p_4k)
                        warped_n_y = warp_tensor_by_grid(t_n_y, flow_n_4k)
                        
                        cons_guide = F.interpolate(flow_consistency_t, size=(2160, 3840), mode='bilinear', align_corners=False)
                        epi_p_guide = F.interpolate(epi_p_cv_t, size=(2160, 3840), mode='bilinear', align_corners=False)
                        epi_n_guide = F.interpolate(epi_n_cv_t, size=(2160, 3840), mode='bilinear', align_corners=False)
                        lap_guide = get_laplacian_map(t_up_y)

                        fused_y, luma_weights = luma_model(t_up_y, warped_p_y, warped_n_y, cons_guide, lap_guide, epi_p_guide, epi_n_guide)

                        # 🌟 3. Chroma (U/V) 奴役化融合 (0 NN MACs!)
                        flow_p_2k = scale_flow(flow_p_t_refined, 1080, 1920)
                        flow_n_2k = scale_flow(flow_n_t_refined, 1080, 1920)
                        warped_p_u = warp_tensor_by_grid(t_p_u, flow_p_2k)
                        warped_n_u = warp_tensor_by_grid(t_n_u, flow_n_2k)
                        warped_p_v = warp_tensor_by_grid(t_p_v, flow_p_2k)
                        warped_n_v = warp_tensor_by_grid(t_n_v, flow_n_2k)
                        
                        # 直接把 Luma 網路算出來的權重，縮小一半套在 U 和 V 上
                        chroma_weights = F.interpolate(luma_weights, size=(1080, 1920), mode='bilinear', align_corners=False)
                        
                        fused_u = (chroma_weights[:, 0:1] * t_up_u) + (chroma_weights[:, 1:2] * warped_p_u) + (chroma_weights[:, 2:3] * warped_n_u)
                        fused_v = (chroma_weights[:, 0:1] * t_up_v) + (chroma_weights[:, 1:2] * warped_p_v) + (chroma_weights[:, 2:3] * warped_n_v)
