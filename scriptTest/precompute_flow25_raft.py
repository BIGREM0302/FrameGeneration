...
print("Loading SOTA RAFT Flow Model...")
...
def get_raft_flow(img1_small, img2_small):
    pad_h = 544 - img1_small.shape[2]
    i1_pad = F.pad(img1_small, (0, 0, 0, pad_h), mode='replicate')
    i2_pad = F.pad(img2_small, (0, 0, 0, pad_h), mode='replicate')
    i1_rgb = i1_pad.repeat(1, 3, 1, 1) * 2.0 - 1.0
    i2_rgb = i2_pad.repeat(1, 3, 1, 1) * 2.0 - 1.0
    with torch.no_grad():
        flow = raft_model(i1_rgb, i2_rgb)[-1]
    return flow[:, :, :img1_small.shape[2], :]
...
def main():
    ...
    out_dir = "./lightweight_precomputed_flow_raft"
    ...
    for task in train_tasks:
        ...
        pbar = tqdm(range(frameCount), desc=f"RAFT Flow {task['name']}_QP{task['qp']}")
        for t in pbar:
            ...
            with torch.no_grad():
                ...
                flow_pb_t = get_raft_flow(t_p_small, t_up_small)
                flow_nb_t = get_raft_flow(t_n_small, t_up_small)
            ...
            flow_dict = {
                'fp': flow_p_t.cpu(), 'fn': flow_n_t.cpu(),
                'fpb': flow_pb_t.cpu(), 'fnb': flow_nb_t.cpu(),
