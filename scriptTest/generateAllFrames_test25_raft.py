...
FUSION_MODEL_PATH = "joint_fusion_net_v25_raft_best.pth"
REFINE_MODEL_PATH = "flow_refine_net_v25_raft_best.pth"
...
ml_model = LiteMicroFusionNetV25().to(device)
            ...
            for qp in config['qps']:
                ...
                with open(out_file, 'wb') as f_out:
                    for t in tqdm(range(frameCount), desc=f"Video: {video_name} QP: {qp}", unit="frame"):
                        ...
                        flow_p_back_cv = flow_data_pt['fpb'].float().to(device)
                        flow_n_back_cv = flow_data_pt['fnb'].float().to(device)
