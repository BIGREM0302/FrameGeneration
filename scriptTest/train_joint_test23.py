...
FUSION_MODEL_PATH = "joint_fusion_net_test23.pth"
REFINE_MODEL_PATH = "flow_refine_net_test23.pth"
FUSION_MODEL_BEST_PATH = "joint_fusion_net_best23.pth"
REFINE_MODEL_BEST_PATH = "flow_refine_net_best23.pth"
PRECOMPUTED_DIR = "./precomputed_flow_pt"
LOSS_PLOT_PATH = "loss_curve23.png"
...
def draw_loss_curve_cv2(epochs, losses, save_path):
    ...
    for i, (ep, loss) in enumerate(zip(epochs, losses)):
        ...
        # Y 座標根據 Loss 值域進行線性映射 (加上下 5% 的 padding)
        pad = loss_range * 0.05
...
class MicroFusionNet(nn.Module):
    def __init__(self):
        ...
        # 🌟 將初始溫度降為 1.0，避免 Softmax 初期飽和導致梯度消失
...
def main():
    ...
    eps = 1e-6
    # 🌟 下調 LR 到 2e-4，防止震盪
    optimizer = optim.AdamW(list(fusion_model.parameters()) + list(refine_model.parameters()), lr=2e-4, weight_decay=1e-4)
